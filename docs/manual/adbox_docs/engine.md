...
Currently, the ADBox engine supports the [MTAD-GAT](./mtad_gat.md) algorithm for anomaly detection.

Instructions are given to the engine as [use cases](./use_case.md). The engine can parse and transform use case input into data consumable by various detection pipelines, by calling the [`DetectorsConfigManager`](/siem_mtad_gat/config_manager/config_manager.py).
