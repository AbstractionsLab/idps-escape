...
If nothing is specified, the [prediction pipeline](./engine.md#prediction-pipeline) outcomes are stored in the corresponding [detector folder](./detector_data_structure.md).
...
The training and prediction pipelines are instructed via [use cases](./use_case.md). To enable data shipping to the Wazuh indexer, it is sufficient to run ADBox with the flag `-s`:
...
![](../_figures/1BA5_Tutorial_Dashboard/1BA5_3_data_streams.png)
