...
└─────────────────────────────────────────────────────────────┘
        ...
        ┌───────────────────┬───────────────────┬───────────────────┐
        ↓                   ↓                   ↓                   ↓
   DOCKER_LOCAL     DOCKER_REMOTE         HOST_REMOTE       (Selection)
   (Local Dev)      (Remote Container)    (Bare Metal)
```

The playbook branches into three **mutually exclusive blocks** based on the `manager_mode` variable set in inventory:
