...
ref: ''
release: alpha
reviewed: xMwgK17qXCIN2OMjiNuMkKR20ZwlgyBLmkK1xvRxPsw=
success_criteria: |
...
# ADBox Create detector data stream
...
```
- Historical prediction: the start time shall be the present day at '00:06:00Z' (notice *window size* is 6 and *granularity* 1min) and the end time/present timestamp rounded to the closest
*unit timestamp*.
```
...
#### Wazuh Dashboard:
  ...
  - **Indexer Management>Data streams** check if `adbox_detector_mtad_gat_{detector_id}` is there and contains at least a document.
      ![Dashboard](assets/TST-18-detector-stream.png){: width="80%"}