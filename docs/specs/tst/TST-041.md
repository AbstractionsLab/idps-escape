...
- Targeted Wazuh manager and agent versions: `4.12.0`
...
- If the agents are to be deployed on `remote` nodes, the Wazuh agents must be installed in the monitored endpoints using [the official documentation](https://documentation.wazuh.com/4.12/installation-guide/wazuh-agent/wazuh-agent-package-linux.html).
- The agent must be registered with the Wazuh manager following [the official documentation](https://documentation.wazuh.com/4.12/user-manual/agent/agent-enrollment/enrollment-methods/via-agent-configuration/linux-endpoint.html).
