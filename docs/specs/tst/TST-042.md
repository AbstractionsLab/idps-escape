...
1. In the controller host, the working directory is required to be `soar-radar`:
```
cd soar-radar
```
...
```
to ensure that files from `soar-radar/suspicious_login/active_responses/*` are present under `/var/ossec/active-response/bin/` with owner `root:wazuh` and mode `0750`.
