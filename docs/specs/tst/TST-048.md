...
ref: ''
release: alpha
reviewed: oTY1m2WqcU15CVAlIblmOc9midqoXQWofJr4SuR-nUI=
test_data: see referenced files
verification_method: T
version: 0.8
---

# Detection validation for Log volume abnormal growth

## Preconditions and setup actions
- RADAR is setup according to [TST-045](./TST.html#TST-045)
...
2.5. Verify that the spike file  (`log_volume.target_dir / log_volume.spike_filename`) is removed after `cleanup_minutes` have elapsed (if `cleanup_minutes > 0`), or remove it manually if `cleanup_minutes` is set to 0.