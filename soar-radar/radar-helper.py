...
def main():
    DEBUG_LOGGER.debug(
        "[main] creating watchers for AUTH_LOG=%s, AUDIT_LOG=%s",
        AUTH_LOG, AUDIT_LOG
    )
    ...
    try:
        ...
        DEBUG_LOGGER.debug("[main] AuthLogWatcher created successfully")
    except Exception as e:
        DEBUG_LOGGER.error(
            "[main] FAILED to create AuthLogWatcher: %s",
            e,
            exc_info=True
        )
        raise

    watchers = [auth_watcher]
