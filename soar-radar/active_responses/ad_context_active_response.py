...
WAZUH_AUTH_PASS  = "<wazuh-wui-password>"
...
def send_agent_ar(agent_id, cmd, args, token, alert_data, timeout=None):
...
def main():
    ...
    if action == "add" and user and start and end:
        ...
        for pc, events in by_pc.items():
            ...
            send_agent_ar(aid, "!write_contextual_logs_active_response.sh", [user, events_json], token, data, timeout=None)

        # Optional lockout (comment out to disable)
        for pc in by_pc:
            aid = get_agent_id_by_pc(pc, token)
            if aid:
                send_agent_ar(aid, "!lock_user_linux_active_reponse.sh", [user], token, data, timeout=60)
