...
```
And paste the content of `radar-agent-ossec-snippet.xml` inside of `<ossec_config>` tag.
...
1. Copy the content of the decoder (`local_decoder.xml`) into the `/var/ossec/etc/decoders/local_decoder.xml`
2. Copy the content of the rules (`local_rules.xml`) into the `/var/ossec/etc/rules/local_rules.xml`
3. Configure active-response scripts.
```
cp active_responses/email_ar.py /var/ossec/active-response/bin/email_ar.py
chmod 750 /var/ossec/active-response/bin/email_ar.py
chown root:wazuh /var/ossec/active-response/bin/email_ar.py
```
4. Configure commands and active responses by copying the content of `radar-ossec-snippet.xml` into the end of `/var/ossec/etc/ossec.conf` under `<ossec_config>` tag.
...
Under **Categorical field**, select the user identifier `agent.name.keyword`.
