...
# (normalize_timestamp and extract_timestamp) from utils.py to ensure STIX2-compliant formatting.
#
...
def safe_ip(ip_str):
    try:
        ...
        log.warning(f"Ignoring invalid IP: {ip_str}")
        return None
...
def vulnerability_active(sightings: SightingsCollector) -> bool:
    ...
    last_seen = {
        sighter: {
            rule_id: max((extract_timestamp(a["_source"]) for a in alerts))
...
class WazuhConnector:
    ...
    def _process_message(self, data):
        ...
        for hit in hits:
            try:
                ...
                required_keys = ("rule", "agent")
                timestamp_keys = ("@timestamp", "timestamp")

                # Check required fields
                # if not all(k in s for k in ("rule", "agent")): #"@timestamp",
                if not all(k in s for k in required_keys):
                    log.warning(f"Skipping malformed alert (missing required keys: {required_keys}): {json.dumps(s)}")
                    continue

                if not any(k in s for k in timestamp_keys):
                    log.warning(f"Skipping malformed alert (missing timestamp field: {timestamp_keys}): {json.dumps(s)}")
                    continue

                if "id" not in s["rule"]:
                    log.warning(f"Skipping alert: missing rule.id in {json.dumps(s['rule'])}")
                    continue

                if not all(k in s["agent"] for k in ("id", "name")):
                    log.warning(f"Skipping alert: incomplete agent data in {json.dumps(s['agent'])}")
                    continue

                rule_id = s["rule"]["id"]
                if rule_id in self.conf.rule_exclude_list:
                    log.info(f"Ignoring alert rule id {rule_id} because it is in rule_exclude_list")
                    continue

                if self.conf.agents_as_systems and int(s["agent"]["id"]) > 0:
                    agents[s["agent"]["id"]] = sighter = self.create_agent_stix(hit)
                else:
                    sighter = self.siem_system

                timestamp = extract_timestamp(s) #s.get("@timestamp", s.get("timestamp"))
                ...
                raise OpenSearchClient.ParseError(
                    "Failed to parse _source: Unexpected JSON structure {json.dumps(hit)}"
                ) from e
    ...
    def create_sighting_stix(
        ...
        return stix2.Sighting(
            ...
            **self.stix_common_attrs,
            first_seen=normalize_timestamp(metadata.first_seen),
            last_seen=normalize_timestamp(metadata.last_seen),
    ...
    def create_alert_note(
        ...
        *,
        ...
        s = alert["_source"]
        sighted_at = extract_timestamp(s) #s.get("@timestamp") or s.get("timestamp")  #s["@timestamp"]
            ...
            case Config.IncidentCreateMode.PerAlert:
                for sighter_id, meta in sightings_meta.alerts_by_sighter_meta().items():
                    ...
                    incidents = [
                        ...
                        for alert in meta["alerts"]
                        for sighted_at in (extract_timestamp(alert["_source"]),) #["@timestamp"]
    ...
    def create_agent_addr_obs(self, *, alerts: list[dict]):
        ...
        bundle = []
        timestamps = [extract_timestamp(alert["_source"]) for alert in alerts]

        # Convert all timestamps to normalized strings
        normalized_timestamps = [normalize_timestamp(ts) for ts in timestamps]
        ...
        for agent in agents.values():
            if agent["ip"] is None:
                log.warning(f"Skipping agent {agent['name']} because IP is missing or invalid")
                continue
            ...
            if "scan_time" in agent:
                scan_time = agent["scan_time"]
                # Assume scan_time is datetime; convert to ISO format
                scan_time_str = normalize_timestamp(scan_time.isoformat())
                if latest is None or scan_time_str > latest:
                    stop_time = scan_time_str
    ...
    def create_agent_hostname_obs(self, *, alerts: list[dict]):
        ...
        bundle = []
        earliest = min(extract_timestamp(alert["_source"]) for alert in alerts) # alert["_source"]["@timestamp"]
        latest = max(extract_timestamp(alert["_source"]) for alert in alerts) # alert["_source"]["@timestamp"]
        # STIX doesn't accept start == stop, so remove stop if they are the same:
        if latest == earliest:
            latest = None

        for agent in agents.values():
            if not agent.get("hostname"):
                log.warning(f"Skipping agent {agent['name']} because hostname is missing")
                continue

            hostname = stix2.DomainName(
            ...
            normalized_earliest = normalize_timestamp(earliest) if earliest else None
            normalized_latest = normalize_timestamp(latest) if latest else None

            # handle stop_time
            stop_time = normalized_latest
            if "scan_time" in agent:
                scan_time = agent["scan_time"]
                scan_time_str = normalize_timestamp(scan_time.isoformat())
                if normalized_latest is None or scan_time_str > normalized_latest:
                    stop_time = scan_time_str
            ...
            rel = stix2.Relationship(
                ...
                **self.stix.common_properties,
                ...
                start_time=(None if agent.get("is_new") else normalized_earliest),
                stop_time=stop_time,
