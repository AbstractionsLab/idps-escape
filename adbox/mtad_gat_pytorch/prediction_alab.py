...
import pandas as pd
import torch
#from adbox.mtad_gat_pytorch.eval_methods import *
from adbox.mtad_gat_pytorch.eval_methods import adjust_predicts, epsilon_eval, find_epsilon, pot_eval
from adbox.mtad_gat_pytorch.utils import SlidingWindowDataset
from adbox.data_manager.data_storage_manager import DataStorageManager
from adbox.commons import EscapeError
from adbox.mtad_gat_pytorch import *
...
class Predictor:
    ...
    def __init__(self, model, window_size, n_features, pred_args, batch_size : int = 256):
        ...
        self.target_dims = pred_args.get(keys.MTAD_GAT_TARGET_DIMS)
        self.scale_scores = pred_args.get(keys.MTAD_GAT_SCALE_SCORES)
        self.q = pred_args.get(keys.MTAD_GAT_Q)
        self.level = pred_args.get(keys.MTAD_GAT_LEVEL)
        self.dynamic_pot = pred_args.get(keys.MTAD_GAT_DYNAMIC_POT)
        self.use_mov_av = pred_args.get(keys.MTAD_GAT_USE_MOV_AV)
        self.gamma = pred_args.get(keys.MTAD_GAT_GAMMA)
        self.reg_level = pred_args.get(keys.MTAD_GAT_REG_LEVEL)
        self.batch_size = batch_size
        self.use_cuda = True
        self.pred_args = pred_args

    def get_score(self, values) -> pd.DataFrame:
        ...
        loader = torch.utils.data.DataLoader(data, batch_size=self.batch_size, shuffle=False) # type: ignore
        ...
        for i in range(preds.shape[1]):
            df_dict[keys.MTAD_GAT_OUT_FORECAST_i.format(i=i)] = preds[:, i]
            df_dict[keys.MTAD_GAT_OUT_RECON_i.format(i=i)] = recons[:, i]
            df_dict[keys.MTAD_GAT_OUT_TRUE_i.format(i=i)] = actual[:, i]
            ...
            df_dict[keys.MTAD_GAT_OUT_SCORE_i.format(i=i)] = a_score

        df = pd.DataFrame(df_dict)
        anomaly_scores = np.mean(anomaly_scores, 1)
        df[keys.MTAD_GAT_OUT_SCORE_GLOBAL] = anomaly_scores
    ...
    def predict_anomalies(self, 
                          ...
                          test, 
                          true_anomalies=None, 
                          train_timestamps:pd.Index=pd.Index([]), 
                          test_timestamps:pd.Index=pd.Index([]), 
                          load_scores : bool = False, 
                          save_output : bool = True,
                          scale_scores:bool=False,
                          only_predict:bool=False,
                          method:str=keys.MTAD_GAT_POT_METHOD)-> tuple[pd.DataFrame,pd.DataFrame]:
        ...
        if load_scores:
            ...
            train_pred_df : pd.DataFrame = self.get_score(train) 
            test_pred_df : pd.DataFrame | None = self.get_score(test) if test!=None else None
            

            train_anomaly_scores  = train_pred_df.get(keys.MTAD_GAT_OUT_SCORE_GLOBAL).values
            test_anomaly_scores = test_pred_df.get(keys.MTAD_GAT_OUT_SCORE_GLOBAL).values if test!=None else None # type: ignore
            ...
            # Update df
            train_pred_df[keys.MTAD_GAT_OUT_SCORE_GLOBAL] = train_anomaly_scores
            if test!=None: test_pred_df[keys.MTAD_GAT_OUT_SCORE_GLOBAL] = test_anomaly_scores  # type: ignore
        ...
        for i in range(out_dim):
            train_feature_anom_scores = train_pred_df.get(keys.MTAD_GAT_OUT_SCORE_i.format(i=i)).values
            test_feature_anom_scores = test_pred_df.get(keys.MTAD_GAT_OUT_SCORE_i.format(i=i)).values if test != None else  None # type: ignore

            if method==keys.MTAD_GAT_POT_METHOD:
                ...
                train_pred_df[keys.MTAD_GAT_OUT_THRESHOLD_i.format(i=i)] = th_pot if only_predict else init_th
                if test != None : test_pred_df[keys.MTAD_GAT_OUT_THRESHOLD_i.format(i=i)] = th_pot # type: ignore

            elif method==keys.MTAD_GAT_EPSILON_METHOD:     
                ...
                train_pred_df[keys.MTAD_GAT_OUT_THRESHOLD_i.format(i=i)] = epsilon
                if test != None : test_pred_df[keys.MTAD_GAT_OUT_THRESHOLD_i.format(i=i)] = epsilon  # type: ignore
            else:
                raise EscapeError(f"Invalid prediction method. method is {method}.",logger)

            train_pred_df[keys.MTAD_GAT_OUT_PREDICTION_i.format(i=i)] = train_feature_anom_preds
            if test != None : test_pred_df[keys.MTAD_GAT_OUT_PREDICTION_i.format(i=i)] = test_feature_anom_preds  # type: ignore
        ...
        if method==keys.MTAD_GAT_POT_METHOD:
            ...
            if test != None: test_pred_df[keys.MTAD_GAT_OUT_TRUE_GLOBAL] = true_anomalies # type: ignore

            train_pred_df[keys.MTAD_GAT_OUT_THRESHOLD_GLOBAL] = th_pot if only_predict else init_th
            if test != None:  test_pred_df[keys.MTAD_GAT_OUT_THRESHOLD_GLOBAL] = init_th # type: ignore
            train_pred_df[keys.MTAD_GAT_OUT_PREDICTION_GLOBAL] =train_anom_preds
            if test != None: test_pred_df[keys.MTAD_GAT_OUT_PREDICTION_GLOBAL] = test_anom_preds # type: ignore
            print()
        else:
            e_eval = epsilon_eval(train_anomaly_scores, test_anomaly_scores, true_anomalies, reg_level=self.reg_level)
            global_epsilon = e_eval["threshold"]
            if test != None: test_pred_df[keys.MTAD_GAT_OUT_TRUE_GLOBAL] = true_anomalies # type: ignore
            train_pred_df[keys.MTAD_GAT_OUT_THRESHOLD_GLOBAL] = global_epsilon
            if test != None:  test_pred_df[keys.MTAD_GAT_OUT_THRESHOLD_GLOBAL] = global_epsilon # type: ignore
            train_pred_df[keys.MTAD_GAT_OUT_PREDICTION_GLOBAL] = (train_anomaly_scores >= global_epsilon).astype(int)
            ...
            if test != None: test_pred_df[keys.MTAD_GAT_OUT_PREDICTION_GLOBAL] = test_preds_global # type: ignore

        #Add timestamps as index

        train_pred_df[keys.MTAD_GAT_OUT_WINDOW]=train_timestamps
        train_pred_df.set_index(keys.MTAD_GAT_OUT_WINDOW, inplace=True)
        if test != None: 
            test_pred_df[keys.MTAD_GAT_OUT_WINDOW]=test_timestamps # type: ignore
            test_pred_df.set_index(keys.MTAD_GAT_OUT_WINDOW, inplace=True) # type: ignore
        print("-- Done.")
