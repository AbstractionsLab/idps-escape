#!/usr/bin/env bash
# manager-assign-agent-group.sh
# Usage:
#   manager-assign-agent-group.sh --scenario <name> [--agent-name <name>] [--agent-ip <ip>]
...
PYTHONPATH="$RADAR_ROOT" python3 -m wazuh_api.cli assign-hosts-to-groups \
  --hosts "$HOSTS_ARG" --scenario "$SCENARIO" --radar-root "$RADAR_ROOT"

echo ""
echo "=== SUCCESS: group assignment for '${SCENARIO}' completed ==="