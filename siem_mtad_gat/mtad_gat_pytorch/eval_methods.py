...
from siem_mtad_gat.mtad_gat_pytorch.spot import SPOT, dSPOT
...
def pot_eval(init_score:np.ndarray, score:np.ndarray, label:np.ndarray, only_predict:bool=True, q:float=1e-3, level:float=0.99, dynamic:bool=True, feature:int=-1):
    """
    Run POT method on given score.
    :param init_score (np.ndarray): The data to get init threshold.
                    For `OmniAnomaly`, it should be the anomaly score of train set.
                    For MTAD_GAT should be the scores of training or prediction inpug
    :param: score (np.ndarray): The data to run POT method.
                    For `OmniAnomaly`, it should be the anomaly score of test set. Same for MTAD_GAT.
    ...
    :only_predict (bool): True if the prediction is base on an already calibrated SPOT object
    :feature (int): Default -1 indicates  global score, else feature scores.
    :return dict: pot result dict
    Method modified from OmniAnomaly (https://github.com/NetManAIOps/OmniAnomaly)
    #Dynamic must be True for ADBox
    """

    print(f"Running POT with q={q}, level={level}..")

    spot_manager=SPOTManager()
    s=spot_manager.spot_list[feature]
    print("init_th:", s.init_threshold)
    if only_predict:
        s.add(init_score) # in predict case this are the data, while score is None
        start=s.data.size-init_score.size #restart from last observation
    else:
    ...
    print("init_th:",s.init_threshold)
    ret = s.run(dynamic=dynamic, with_alarm=True,start=start)
    return ret.get("thresholds"),ret.get("alarms"),s.init_threshold
"""
    ...
    pred, p_latency = adjust_predicts(score, label, pot_th, calc_latency=True) # if label is None : predict = score > pot_th  and latency is None
